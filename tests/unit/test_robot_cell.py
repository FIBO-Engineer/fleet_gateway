...
from fleet_gateway.api.types import RobotCell, Job, OrderStatus, JobOperation, Node
...
@pytest.fixture
def sample_node():
    return Node(id=1, alias="test_node", tag_id="tag1", x=0.0, y=0.0, height=0.0, node_type="PICKUP")
...
@pytest.mark.asyncio
async def test_robot_cell_holding_job(sample_job):
    robot_cell = RobotCell(height=1.0, holding=sample_job, holding_uuid=sample_job.uuid)
    assert robot_cell.holding == sample_job
